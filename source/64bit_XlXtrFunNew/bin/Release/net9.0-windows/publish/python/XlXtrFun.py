...
def _get_derivative_components(ArrayOfXs, ArrayOfYs, GivenX):
    """Pomocná funkcia pre dydx a ddydx."""
    x, y, n = np.array(ArrayOfXs), np.array(ArrayOfYs), len(ArrayOfXs)
    if n < 3: raise ValueError("Derivative calculation requires at least 3 points.")
    ...
    p_left_coeffs, p_right_coeffs = np.polyfit(x_left, y_left, 2), np.polyfit(x_right, y_right, 2)
    y_left_val, y_right_val = np.polyval(p_left_coeffs, GivenX), np.polyval(p_right_coeffs, GivenX)
    dp_left_coeffs, dp_right_coeffs = np.polyder(p_left_coeffs), np.polyder(p_right_coeffs)
    dy_left_val, dy_right_val = np.polyval(dp_left_coeffs, GivenX), np.polyval(dp_right_coeffs, GivenX)
    ddp_left_coeffs, ddp_right_coeffs = np.polyder(p_left_coeffs, 2), np.polyder(p_right_coeffs, 2)
    ddy_left_val, ddy_right_val = np.polyval(ddp_left_coeffs, GivenX), np.polyval(ddp_right_coeffs, GivenX)
    knot_left_x, knot_right_x = x[i], x[i+1]
    h = knot_right_x - knot_left_x
    if h == 0: return (y[i], 0, y_left_val, y_right_val, dy_left_val, dy_right_val, ddy_left_val, ddy_right_val)
    weight, d_weight = (GivenX - knot_left_x) / h, 1 / h
    return (weight, d_weight, y_left_val, y_right_val, dy_left_val, dy_right_val, ddy_left_val, ddy_right_val)
...
def PFitData(ArrayOfXs, ArrayOfYs, Order, notRequireGoThrough00=True):
    """
    ...
    Args:
        ...
        notRequireGoThrough00 (bool): Ekvivalent `const` v LINEST. Ak False, krivka prechádza (0,0).
    ...
    # F-štatistika
    df_reg = p - 1 if notRequireGoThrough00 else p
